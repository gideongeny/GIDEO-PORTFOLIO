  ...
  function displayGitHubRepositories(repos) {
    ...
    container.innerHTML = repos.map(repo => {
      ...
      if (homepageUrl && homepageUrl.startsWith('http')) {
        // Try to get actual website screenshot using thum.io (works better for live websites)
        // This service can capture actual website screenshots
        imageSrc = `https://image.thum.io/get/width/1280/crop/720/noanimate/${homepageUrl}`;
      }
      ...
      return `
        <div class="project-card github-repo-card">
          <div class="repo-image-container" style="background: ${gradient}; position: relative; overflow: hidden;">
            <img src="${imageSrc}" alt="${repo.name}" loading="lazy" crossorigin="anonymous" referrerpolicy="no-referrer" onerror="const self = this; let attempt = self.dataset.attempt || 0; attempt++; self.dataset.attempt = attempt; if(attempt === 1 && (self.src.includes('thum.io') || self.src.includes('screenshot'))) { self.src = '${fallbackImageUrl}'; return; } this.onerror=null; this.style.display='none'; const placeholder = this.nextElementSibling; if(placeholder) placeholder.style.display='flex';" style="width: 100%; height: 100%; object-fit: cover; position: absolute; top: 0; left: 0; z-index: 1;">
